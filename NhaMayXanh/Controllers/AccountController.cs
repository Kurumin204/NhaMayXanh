...
namespace WebFigure.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private readonly Dictionary<string, string> _verificationTokens = new Dictionary<string, string>();
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    using(_context)
                    {
                        ...
                        var emails = _context.TbCustomers.Select(x => x.Email).ToList();
                        var khachhang = _context.TbCustomers.
                            AsNoTracking().
        ...
        private async Task SendVerificationEmail(string email)
        {
            ...
            // Store the verification token for later verification (replace with your storage mechanism)
            _verificationTokens.Add(email, verificationToken); // Replace _verificationTokens with your storage solution (e.g., database)

            // Create the verification link with the token
            string verificationLink = Url.Action("VerifyEmail", "Account", new { token = verificationToken }, protocol: HttpContext.Request.Scheme);

            // Build the email content with the verification link
            string emailBody = $"Thanks for registering for our service! Please click the link below to verify your email address: <br/> {verificationLink}";

            // Send the email using your preferred email service
            await _emailService.SendEmailAsync(email, "Verify Your Email Address", emailBody);
        }
